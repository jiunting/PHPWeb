...
$station = mysqli_real_escape_string($conn, $station);
...
$query = "SELECT e.eventID, e.originTime, e.longitude, e.latitude, e.depth, e.magnitude FROM earthquake e JOIN earthquake_has_station es USING(eventID) JOIN station s USING(stationID) JOIN network n ON s.network_ID=n.ID WHERE n.ID='";
$query = $query . $networkID .  "' AND s.code='" . $station ;
$query = $query . "' ORDER BY e.originTime LIMIT 500;"
...
<p>
<a href="findEQinSta.txt" >Contents</a>
